...
|---|---|---|---|
| **1 — Wedge** | 1–3 | Client Delivery (notes→summary, prep briefs, timeline), basic scheduling embed *(powered by our self-hosted, forked Cal.com — running from day one on Railway, doc 07 §6/doc 03 row 1, not deferred to Phase 2)*, client portal | 5 pilot coaches using it weekly; ≥3 convert to paid |
| **2 — Money loop** | 4–8 | Bookings module matures into its full feature set (calendar sync, reminders, richer scheduling — same underlying self-hosted Cal.com fork, more of it exposed), Contracts (Documenso, self-hosted/forked, doc 03 row 2), Stripe billing; **Gmail integration** (start Google's verification review in parallel, month 4 — see risk below); security re-review | ≥25 paying coaches; churn <5%/mo; coaches running real revenue through us |
| **3 — Growth loop** | 9–14 | Leads CRM (built natively — Twenty dropped, doc 03 row 3/doc 09 §3), AI follow-ups + the Practice Agent (Mastra, doc 10), marketing composer, landing pages, **Automations v1** (Module 8, on Activepieces — replaced n8n, doc 03 row 5/doc 09 §5); done-for-you service launch; **Coach & Mentor Directory** built but held unlaunched; **Daily Briefing** (Module 10) — in-app and public `/briefing` page; **Admin MCP server** (F20) | ≥100 paying coaches; service line profitable; Directory opens to public search only once coach supply clears the same ≥100 gate |
...
| Activepieces (Automations engine — replaced n8n, doc 03 row 5) | $0 | MIT-licensed, self-hosted; new templates only |
