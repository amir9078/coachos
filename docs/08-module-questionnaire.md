...
- **M12-Q19.** ✔ **ANSWERED — the deliberate expansion this rule anticipated.** The Pipeline Agent is generalized into a **Practice Agent** with reach across Leads/Pipeline, Client Delivery, Bookings, Billing, and Autopilot state (doc 10 §6, doc 02 Module 7) — requested explicitly by the founder, not something the agent grew into on its own. The cap moves from "Leads only" to this defined, still-bounded list; it is not open-ended access to every table. Any future expansion (e.g. into Directory or Community once built) needs the same explicit sign-off. The approval-first floor is unchanged — wider read access never means wider send/act authority.
- **M12-Q20** *(new)*. ✔ **ANSWERED.** Should the Practice Agent's reach be a single global on/off, or customizable per module? *(per module — a new "Practice Agent Rules" settings panel, one row per module, with on/off, daily-scan-vs-on-demand, and per-insight-type toggles, default-on across the board. Full detail: doc 10 §8. Cost estimate for the wider scope, grounded in doc 04's Haiku 4.5 unit economics: ≈$0.65–$0.70/coach/month, ~20x the $0.03/coach/month baseline for the older single-call AI features but still under 2% of the $59/mo Practice-tier price point — same doc, same section.)*
