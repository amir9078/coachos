...
**Cal.com, Documenso, and Listmonk, all AGPLv3 — resolved: modify freely across all three, knowingly accepting the compliance risk.** Cal.com was the first of these decided explicitly; the founder then extended the same call to Documenso and Listmonk too ("modify everything... even rename the engine to ours and change every branding to ours"). All three will be fully customized/forked, rebranded to CoachOS throughout, and run entirely on CoachOS's own infrastructure (Railway → Coolify), with the modified code kept private rather than published. AGPLv3 requires publishing the source of a modified version to anyone who can interact with it over a network — none of these three follow that requirement, and none purchase a commercial license that would remove it either (Cal.com has one and it wasn't bought; Documenso may have an enterprise option, unconfirmed; Listmonk likely has no such option to buy at all). **This was a deliberate, informed choice, made after the disclosure obligation was explained directly for Cal.com first** (not an oversight, and not something that crept in gradually): the founder explicitly said, of Cal.com, "I want to modify the code completely, and keep it only with me... if I would face any legal consequences, I would take care of it" — and then explicitly chose to apply the same approach to the other two rather than treating them differently. Recorded here precisely for that reason — if legal counsel ever reviews this decision, this is the actual reasoning and risk accepted for all three engines, not a cleaned-up version of it. **Activepieces (MIT) and LiveKit (Apache 2.0) are a different case entirely** — both can be forked and rebranded with zero legal question, since neither license requires anything back. Nothing is being "accepted as risk" for those two; there simply isn't any.
