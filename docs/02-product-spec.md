...
| Action-button philosophy | Confirmed — every insight ships with a next action, never a bare number. **This is now literally powered by the Practice Agent** (doc 10 §6), not just threshold rules — the agent reasons across leads, client delivery, bookings, and billing to surface what a single-module view can't see, then proposes the action attached to each insight. |
...
### The Practice Agent (generalized from Module 2's Pipeline Agent — full detail in doc 10 §6)

This module's insight engine is a single Mastra-based agent with read access across Leads/Pipeline, Client Delivery, Bookings, Billing, and Autopilot state — not a separate implementation per module. It surfaces cross-module patterns (a package running low with no renewal sent, an overdue invoice ahead of tomorrow's session, content that's gone quiet after it worked) and proposes the next action — but the approval-first floor is unchanged: wider visibility never means wider authority. It can suggest more; it can still never send, post, or charge anything without the coach approving the exact action first. The same agent is invokable contextually from a client's or lead's own page ("check on this client"), reusing the identical tools rather than a second agent.

**Customizable per module, not just globally on/off** (full detail in doc 10 §8): a new "Practice Agent Rules" settings panel, sitting alongside the AI Voice & Rules Center, lets the coach turn each module's data access on/off, set whether it's checked in the daily scan or only on-demand, and toggle which specific insight types are active per module — all default-on, coach opts out per module rather than opting in. The approval-first floor is unaffected by any of these toggles; they control what the agent can see and surface, never what it's allowed to do unattended.
