...
| 2 | **Documenso** | github.com/documenso/documenso | AGPLv3 (commercial/enterprise option, if any, not purchased) | Contracts module: e-signatures | 2 *(module timing unchanged — only the licensing/fork decision moved, not the roadmap)* | **Forked and fully customized, rebranded to CoachOS** — same founder decision as Cal.com (below), on our own infrastructure, modifications kept private rather than published as AGPLv3 would require. Same knowingly-accepted risk, see doc 07 §6. |
...
| 4 | **[Listmonk](https://listmonk.app/)** | github.com/knadh/listmonk | AGPLv3 | Newsletter sending, subscriber lists, nurture sequences (Module 3) | 3 *(module timing unchanged — only the licensing/fork decision moved, not the roadmap)* | **Forked and fully customized, rebranded to CoachOS** — same founder decision and knowingly-accepted AGPL risk as Cal.com and Documenso. No commercial license appears to exist for Listmonk to purchase instead (unverified — Listmonk is a single-maintainer community project; if this matters later, confirm directly rather than assume). |
| 5 | ~~n8n~~ **→ Activepieces** | github.com/activepieces/activepieces | **MIT** (Community Edition — verified at the repo) | Automation engine: recipes, scheduled flows, triggers, syncs. Replaced n8n: its Sustainable Use License doesn't cover powering a paid customer-facing feature ([`docs/09`](09-engine-comparison.md) §5, license text quoted) | 2–3 | Self-host + API; coaches only ever see our own recipes UI. **MIT — fully forkable/rebrandable with zero copyleft obligation**, unlike the three AGPL engines above: no risk being accepted here, this one's just genuinely free to customize. |
...
| 16 | **Mastra** | github.com/mastra-ai/mastra | Apache 2.0 (core) | The **Practice Agent** (generalized from the original Leads-only "Pipeline Agent") — multi-step reasoning + human-in-the-loop approval, reading across Leads, Client Delivery, Bookings, Billing, and Autopilot state, running inside our own app, not a separate service | 3 | Library, not a hosted service — full research and demo in [`docs/10-ai-agent-layer.md`](10-ai-agent-layer.md) |
...
| 20 | **LiveKit** | github.com/livekit/livekit | Apache 2.0 | CoachOS Meet — in-platform video, recording, transcription (doc 09 §8) | 1 (test VPS from day one, per docs/08 M4-Q13) | Self-hosted on its own small VPS (native UDP need — Railway/Vercel can't serve this). **Apache 2.0 — fully forkable/rebrandable with zero copyleft obligation**, same free-to-customize status as Activepieces. |
...
Phase 1 (the wedge) needs: **Next.js + Supabase + Claude API + a self-hosted, forked, rebranded Cal.com** — Cal.com specifically ships in Phase 1 because "basic scheduling embed" was already part of the wedge's original scope (doc 04), and it's now self-hosted/forked from day one rather than using Cal.com's hosted product (founder decision, doc 07 §6). **Documenso and Listmonk's actual build timing is unchanged** — Contracts is still Phase 2, Newsletter is still Phase 3. What *did* change for both of them is the licensing/customization approach for whenever they do get built: forked, fully modified, rebranded to CoachOS, kept private, same as Cal.com — decided now, applied later. Don't stand up more self-hosted services than the current phase actually needs; that discipline is about operational liability, not licensing, and still holds regardless of how any given engine is licensed.

## License obligations checklist

- [x] Cal.com: self-hosted, fully modified, **rebranded to CoachOS**, kept private, commercial license not purchased. Knowing, explicit risk acceptance on record — doc 07 §6.
- [x] Documenso: **same treatment as Cal.com** — forked, fully modified, rebranded, kept private. Same knowing risk acceptance, extended explicitly by the founder to this engine too.
- [x] Listmonk: **same treatment again** — forked, fully modified, rebranded, kept private. No commercial-license alternative is known to exist for Listmonk specifically (unverified either way — it's a small community project); if that turns out to matter, confirm directly rather than assume.
- [x] Activepieces, LiveKit: **no obligation to track here at all** — MIT and Apache 2.0 respectively, both fully forkable and rebrandable with zero copyleft requirement. Not a risk being accepted, just genuinely free.
- [ ] Keep our proprietary app code in a separate repository from any AGPL code (Cal.com, Documenso, Listmonk) regardless of the above — this reduces the chance of accidentally pulling the AGPL disclosure obligation onto the *rest* of CoachOS too, which none of these decisions were intended to risk
- [ ] Legal review of the license posture in the same pass as the contracts-template legal review — **should now cover all three AGPL engines' rebranding**, not just Cal.com, given the founder has flagged willingness to address consequences after the fact rather than before, for all three
