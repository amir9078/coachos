...
Phase 1 (the wedge) needs: **Next.js + Supabase + Claude API + self-hosted, forked, rebranded Cal.com, Documenso, and Listmonk** (founder decision, doc 07 §6 — broader than the lightweight-embed/hosted-tier approach originally planned here). Everything else (Activepieces, LiveKit go live as needed per their own module phases) arrives on the same self-host-and-fork basis, since the founder's direction is now "modify everything, rebrand everything" across the whole self-hosted list, not staged in behind hosted tiers. Do not stand up more services than the current phase actually needs, though — each self-hosted service is still an operational liability regardless of licensing, until revenue justifies it.
